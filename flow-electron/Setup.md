...
When Flow detects it's running inside Electron (`window.__flowElectron` exists):
- Gesture coordinates map to your **full screen resolution**
- `cursor_move` → robotjs moves your actual OS mouse cursor
- `gesture_click` → robotjs fires a real left click
- `scroll` → robotjs scrolls whatever is under the cursor
- `right_click` → robotjs fires a real right click
