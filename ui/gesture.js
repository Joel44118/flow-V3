...
// - Canvas fix: ap// ui/gesture.js (v16)
// FIXES:
// - modelComplexity: 0 (was 1) — much faster, less lag
// - EMA alpha 0.5 (was 0.35) — snappier tracking
// - TWO-FINGER SWIPE for scroll (index + middle extended, no thumb)
...
export async function start(videoEl) {
  try {
    ...
    // Tell Electron overlay to show the gesture dot window
    if (window.__flowElectron) window.__flowElectron.send('gesture_start', {});
...
function _onResults(results) {
  ...
  if (!raw) {
    ...
    _ctx.clearRect(0, 0, _canvas.width, _canvas.height);
    // Tell Electron overlay to show held position across full screen
    if (window.__flowElectron) {
      window.__flowElectron.send('cursor_held', {
        x: _curX * (window.screen?.width || window.innerWidth),
        y: _curY * (window.screen?.height || window.innerHeight),
      });
    }
...
export function stop() {
  _active = false;
  // Tell Electron to hide the overlay dot
  if (window.__flowElectron) window.__flowElectron.send('gesture_stop', {});
...
Gesture.start = start;
Gesture.stop  = stop;pended inside video container with inset:0
